...
test.describe('Loading Indicators Tests', () => {
  ...
  test('should show loading indicator when saving grid layout', async ({ page }) => {
    // Navigate to product grid layout management
    await page.click('text=Admin Panel');
    await page.click('text=Product Grid Layout Management');
    
    // Click customize grid layout
    await page.click('button:has-text("Customize Grid Layout")');
    
    // Wait for modal to open
    await page.waitForSelector('text=Customize Product Grid Layout');
    ...
    // Wait for save to complete
    await page.waitForSelector('text=Saving...', { state: 'detached' });
  });

  test('should show loading indicator when deleting grid layout', async ({ page }) => {
    // Navigate to product grid layout management
    await page.click('text=Admin Panel');
    await page.click('text=Product Grid Layout Management');
    ...
    // Find a layout and click delete
    const deleteButton = page.locator('button:has-text("Del")').first();
    await deleteButton.click();
