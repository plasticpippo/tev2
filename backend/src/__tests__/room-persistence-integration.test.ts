...
describe('Room Data Persistence Integration Tests', () => {
  // Clean up the test database before each test
  beforeEach(async () => {
    // Delete all rooms to ensure clean state
  ...
  describe('Room Data Persistence Across Browser Sessions', () => {
    ...
    it('should persist multiple rooms across browser sessions', async () => {
      ...
      expect(retrievedRooms).toHaveLength(2);
      
      // Verify that both rooms exist and have correct data
      const firstRetrievedRoom = retrievedRooms.find((r: any) => r.id === createdRooms[0].id);
      const secondRetrievedRoom = retrievedRooms.find((r: any) => r.id === createdRooms[1].id);
    ...
    it('should persist room with associated tables across browser sessions', async () => {
      // Step 1: Create a room
      const roomData = { 
        name: 'Room With Tables', 
        description: 'A room with associated tables' 
      };
      ...
      const createdRoom = createRoomResponse.body;
      const roomId = createdRoom.id;
      expect(createdRoom.name).toBe(roomData.name);
      expect(createdRoom.tables).toEqual([]);

      // Step 2: Add a table to the room
      const tableData = {
        name: 'Table 1',
        capacity: 4,
        roomId: roomId
      ...
      const createdTable = createTableResponse.body;
      expect(createdTable.roomId).toBe(roomId);
      expect(createdTable.name).toBe(tableData.name);

      // Step 3: Retrieve the room after "browser refresh" - should include the table
      const retrieveRoomResponse = await request(app)
        .get(`/api/rooms/${roomId}`)
        .expect(200);

      const retrievedRoom = retrieveRoomResponse.body;
      expect(retrievedRoom.id).toBe(roomId);
      expect(retrievedRoom.name).toBe(roomData.name);
      expect(retrievedRoom.tables).toHaveLength(1);
      expect(retrievedRoom.tables[0].id).toBe(createdTable.id);
      expect(retrievedRoom.tables[0].name).toBe(tableData.name);
    });
  });
});