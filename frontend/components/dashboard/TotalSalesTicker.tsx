...
export const TotalSalesTicker: React.FC<{ transactions: Transaction[], settings: Settings }> = ({ transactions, settings }) => {
  ...
  const dailyStats = useMemo(() => {
    ...
    const grossSales = roundMoney(todaysTransactions.reduce((sum, t) => {
      const preDiscountTotal = addMoney(addMoney(t.subtotal, t.tax), t.tip);
      return addMoney(sum, preDiscountTotal);
    }, 0));
    ...
    // netSales = grossSales - totalDiscounts (actual revenue collected)
    ...
    return (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title={t('dashboard.totalDiscounts')} value={formatMoney(dailyStats.totalDiscounts)} color="text-purple-400" />
