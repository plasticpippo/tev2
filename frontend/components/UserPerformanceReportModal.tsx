            ...
            <div className="bg-slate-900 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-slate-700">
                <div className="p-6 pb-4 border-b border-slate-700 flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-slate-400">For user: <span className="font-semibold text-white">{user.name}</span></p>
                    </div>
                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <StatCard title="Total Revenue" value={userStats.totalRevenue} />
                        <StatCard title="Transactions" value={userStats.transactionCount} isCurrency={false} />
                        <StatCard title="Average Sale" value={userStats.averageSale} />
                        <StatCard title="Total Tips" value={userStats.totalTips} />
                    </div>

                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h4 className="text-lg font-semibold text-slate-300 mb-2">Top 5 Selling Items</h4>
                            ...
                            ) : (<p className="text-slate-500 text-sm bg-slate-800 p-4 rounded-lg text-center">No items sold.</p>)}
                        </div>
                        <div>
                            <h4 className="text-lg font-semibold text-slate-300 mb-2">Top 5 Removed Items</h4>
                            {userStats.topRemovedItems.length > 0 ? (
                                <ul className="bg-slate-800 p-4 rounded-lg space-y-2">
                                    {userStats.topRemovedItems.map(([name, count]) => (
                                        <li key={name} className="flex justify-between text-sm">
                                            <span className="font-semibold">{name}</span>
                                            <span className="text-red-400">{count} removed</span>
                                        </li>
                                    ))}
                                </ul>
                            ) : (<p className="text-slate-500 text-sm bg-slate-800 p-4 rounded-lg text-center">No items removed.</p>)}
                        </div>
                    </div>

                    <div>
                        <h4 className="text-lg font-semibold text-slate-300 mb-2">Detailed Activity Log</h4>
                        <div className="bg-slate-800 p-2 rounded-lg max-h-64 overflow-y-auto space-y-1">
                            ...
                            )) : <p className="text-slate-500 text-center p-4">No activity logged.</p>}
                        </div>
                    </div>
                </div>

                <div className="flex justify-end p-6 pt-4 mt-auto border-t border-slate-700">
                    <button onClick={onClose} className="bg-slate-600 hover:bg-slate-500 text-white font-bold py-2 px-4 rounded-md">Close</button>
                </div>
            </div>
        </div>
    );
